...
class FullStackEngineerAgent extends BaseAgent {
    ...
    private async loadSystemPrompt() {
        try {
            this.systemPrompt = await fs.readFile(__dirname + '/../prompts/full_stack_engineer_agent.prompt', 'utf-8');
        } catch (error) {
            console.error('Error loading full stack engineer agent system prompt', error);
    ...
    async processTask(task: string, projectId: number, mode: string): Promise<string> {
        try {
            const { type, model, modelName } = getModel(mode, 'FullStackEngineerAgent');
            ...
            const userPrompt = `
                ...
                Based on the following task, decide which tool to use and what the parameters should be. The output should be a JSON object with "tool": "writeFile", "args": {"filePath": "hello.txt", "content": "Hello, World!"}}. Task: ${task}
            `;
            ...
            try {
                ...
                if (parsedResponse.tool && parsedResponse.args) {
                    ...
                    await memoryService.saveMemory('FullStackEngineerAgent', projectId, {
                        decision: `Generated code/plan for task: "${task}"`,
            ...
            console.error(`Error in FullStackEngineerAgent processTask:`, error);
            // Return a structured error object
            return JSON.stringify({
                error: `Task failed for FullStackEngineerAgent.`,
...
export default new FullStackEngineerAgent();
