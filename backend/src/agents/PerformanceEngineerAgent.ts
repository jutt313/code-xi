...
class PerformanceEngineerAgent extends BaseAgent {
    ...
    private async loadSystemPrompt() {
        try {
            this.systemPrompt = await fs.readFile(__dirname + '/../prompts/performance_engineer_agent.prompt', 'utf-8');
        } catch (error) {
            console.error('Error loading performance engineer agent system prompt', error);
    ...
    async processTask(task: string, projectId: number, mode: string): Promise<string> {
        try {
            const { type, model, modelName } = getModel(mode, 'PerformanceEngineerAgent');
            ...
            const userPrompt = `
                ...
                Based on the following task, decide which tool to use and what the parameters should be. The output should be a JSON object with "tool": "executeCommand", "args": {"command": "npm run test:performance"}}. Task: ${task}
            `;
            ...
            const apiCall = () => {
                if (type === 'openai' && model instanceof OpenAI) {
                    ...
                    const geminiModel = model as GenerativeModel; // Explicit cast
                    const combinedPrompt = messages.map(m => m.content).join('\n');
                    return geminiModel.generateContent(combinedPrompt).then(async (result: any) => {
            ...
            try {
                ...
                if (parsedResponse.tool && parsedResponse.args) {
                    ...
                    await memoryService.saveMemory('PerformanceEngineerAgent', projectId, {
                        decision: `Generated performance plan/action for task: "${task}"`,
            ...
            console.error(`Error in PerformanceEngineerAgent processTask:`, error);
            // Return a structured error object
            return JSON.stringify({
                error: `Task failed for PerformanceEngineerAgent.`,
...
export default new PerformanceEngineerAgent();