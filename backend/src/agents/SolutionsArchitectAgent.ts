...
class SolutionsArchitectAgent extends BaseAgent {
    ...
    private async loadSystemPrompt() {
        try {
            this.systemPrompt = await fs.readFile(__dirname + '/../prompts/solutions_architect_agent.prompt', 'utf-8');
        } catch (error) {
            console.error('Error loading solutions architect agent system prompt', error);
    ...
    async processTask(task: string, projectId: number, mode: string): Promise<string> {
        try {
            const { type, model, modelName } = getModel(mode, 'SolutionsArchitectAgent');
            ...
            const userPrompt = `
                ...
                Based on the following task, provide a detailed architectural solution. If you need to perform an action (like reading/writing files or executing commands), output a JSON object with "tool" and "args" properties. For example: {"tool": "writeFile", "args": {"filePath": "architecture.md", "content": "## System Architecture"}}. Task: ${task}
            `;
            ...
            const apiCall = () => {
                if (type === 'openai' && model instanceof OpenAI) {
                    ...
                    return model.messages.create({
                        max_tokens: 2048, // Increased for detailed plans
                        system: systemPrompt,
            ...
            try {
                ...
                if (parsedResponse.tool && parsedResponse.args) {
                    ...
                    // This is an architectural plan, save it.
                    await memoryService.saveMemory('SolutionsArchitectAgent', projectId, {
                        decision: `Generated architectural plan for task: "${task}"`,
                        reasoning: `The model generated the following plan: ${modelResponse.substring(0, 500)}...`,
                        plan: modelResponse
                    });
                    return modelResponse;
                }
            } catch (e) {
                 // If parsing fails, it's a direct text response (the plan itself).
                 await memoryService.saveMemory('SolutionsArchitectAgent', projectId, {
                    decision: `Generated architectural plan for task: "${task}"`,
                    reasoning: `The model generated the following plan: ${modelResponse.substring(0, 500)}...`,
                    plan: modelResponse
            ...
            console.error(`Error in SolutionsArchitectAgent processTask:`, error);
            return JSON.stringify({
                error: `Task failed for SolutionsArchitectAgent.`,
...
export default new SolutionsArchitectAgent();
