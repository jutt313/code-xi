...
class DevOpsEngineerAgent extends BaseAgent {
    ...
    private async loadSystemPrompt() {
        try {
            this.systemPrompt = await fs.readFile(__dirname + '/../prompts/devops_engineer_agent.prompt', 'utf-8');
        } catch (error) {
            console.error('Error loading devops engineer agent system prompt', error);
    ...
    async processTask(task: string, projectId: number, mode: string): Promise<string> {
        try {
            const { type, model, modelName } = getModel(mode, 'DevOpsEngineerAgent');
            ...
            const userPrompt = `
                ...
                Based on the following task, decide which tool to use and what the parameters should be. The output should be a JSON object with "tool": "executeCommand", "args": {"command": "docker-compose up"}}. Task: ${task}
            `;
            ...
            try {
                ...
                if (parsedResponse.tool && parsedResponse.args) {
                    ...
                    await memoryService.saveMemory('DevOpsEngineerAgent', projectId, {
                        decision: `Generated DevOps plan/action for task: "${task}"`,
            ...
            console.error(`Error in DevOpsEngineerAgent processTask:`, error);
            // Return a structured error object
            return JSON.stringify({
                error: `Task failed for DevOpsEngineerAgent.`,
