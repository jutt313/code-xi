...
class DocumentationSpecialistAgent extends BaseAgent {
    ...
    private async loadSystemPrompt() {
        try {
            this.systemPrompt = await fs.readFile(__dirname + '/../prompts/documentation_specialist_agent.prompt', 'utf-8');
        } catch (error) {
            console.error('Error loading documentation specialist agent system prompt', error);
    ...
    async processTask(task: string, projectId: number, mode: string): Promise<string> {
        try {
            const { type, model, modelName } = getModel(mode, 'DocumentationSpecialistAgent');
            ...
            const userPrompt = `
                ...
                Based on the following task, decide which tool to use and what the parameters should be. The output should be a JSON object with "tool": "writeFile", "args": {"filePath": "README.md", "content": "# My Project"}}. Task: ${task}
            `;
            ...
            try {
                ...
                if (parsedResponse.tool && parsedResponse.args) {
                    ...
                    await memoryService.saveMemory('DocumentationSpecialistAgent', projectId, {
                        decision: `Generated documentation plan/content for task: "${task}"`,
            ...
            console.error(`Error in DocumentationSpecialistAgent processTask:`, error);
            // Return a structured error object
            return JSON.stringify({
                error: `Task failed for DocumentationSpecialistAgent.`,
...
export default new DocumentationSpecialistAgent();